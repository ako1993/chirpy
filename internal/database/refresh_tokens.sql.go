...
const revokeRefreshRoken = `-- name: RevokeRefreshRoken :exec
...
func (q *Queries) RevokeRefreshRoken(ctx context.Context, token string) error {
	_, err := q.db.ExecContext(ctx, revokeRefreshRoken, token)
	return err
}
